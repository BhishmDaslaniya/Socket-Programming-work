...
int main(int argc, char** argv){
  ...
  for(int i=0;i<vec_msg_n.size();i++){
    ...
    strcpy(retrv_cmd_char,retrv_cmd_string.c_str());
    //cout<<retrv_cmd_char<<endl;
    status=send(client_fd,retrv_cmd_char,strlen(retrv_cmd_char),0);
    if(status<0){
      cerr<<"Error sending message to server"<<endl;
      exit(5);
    }
